...
export class CapsuleClient {
    ...
    private dekCache: Map<string, { dek: CryptoKey; info: StoredDek }> = new Map();
    ...
    async processAll(): Promise<Map<string, string | Error>> {
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            try {
                ...
                results.set(articleId, err instanceof Error ? err : new Error(String(err)));
            }
        }
     ...
     * ```
     */
    async unlock(article: EncryptedArticle, preferredKeyType: 'tier' | 'article' = 'tier'): Promise<string> {
        const keyPair = await this.ensureKeyPair();

        // Sort wrapped keys by preference
        const sortedKeys = this.sortKeysByPreference(article.wrappedKeys, preferredKeyType);
        ...
        for (const wrappedKey of sortedKeys) {
            ...
            if (cached) {
                ...
                try {
                    ...
                    this.log(`Cached DEK failed for ${wrappedKey.keyId}, trying next`, 'debug');
                }
            }
        }
     ...
     * ```
     */
    async decrypt(article: EncryptedArticle, encryptedDek: string): Promise<string> {
        const keyPair = await this.ensureKeyPair();
    ...
    async clearAll(): Promise<void> {
        ...
        if (this.dekStorage === 'persist') {
            ...
            const keys = Object.keys(sessionStorage).filter((k) => k.startsWith(DEK_STORAGE_PREFIX));
    ...
    private findElement(articleId: string): HTMLElement | null {
        ...
        let element = document.querySelector<HTMLElement>(`[data-capsule-id="${articleId}"]`);
     ...
     */
    private renderContent(element: HTMLElement, content: string, articleId: string): void {
     ...
     */
    private setElementState(element: HTMLElement, articleId: string, state: ElementState): void {
     ...
     */
    private emitEvent<T>(element: HTMLElement, eventName: string, detail: T): void {
     ...
     * Sort wrapped keys by preference.
     */
    private sortKeysByPreference(keys: WrappedKey[], preferredType: 'tier' | 'article'): WrappedKey[] {
     ...
     */
    private async getCachedDek(keyId: string): Promise<{ dek: CryptoKey; info: StoredDek } | null> {
     ...
     */
    private async cacheDek(keyId: string, dek: CryptoKey, response: UnlockResponse): Promise<void> {
        const expiresAt =
        ...
        this.log(`Cached DEK for ${keyId}, expires ${new Date(expiresAt).toISOString()}`, 'debug');
    }

    /**
     * Parse a key ID into type and base ID.
     */
    private parseKeyId(keyId: string): { type: 'tier' | 'article'; baseId: string } {
    ...
    private getDekFromStore(db: IDBDatabase, keyId: string): Promise<StoredDek | null> {
    ...
    private putDekToStore(db: IDBDatabase, keyId: string, info: StoredDek): Promise<void> {
        return new Promise((resolve, reject) => {
     ...
     */
    private async unwrapDek(privateKey: CryptoKey, encryptedDekB64: string): Promise<CryptoKey> {
     ...
     */
    private async decryptWithDek(article: EncryptedArticle, dek: CryptoKey): Promise<string> {
