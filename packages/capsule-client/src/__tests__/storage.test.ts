...
describe('KeyStorage', () => {
  ...
  describe('storeKeyPair / getKeyPair', () => {
    it('stores and retrieves key pair', async () => {
      ...
      await storage.storeKeyPair('test-key', keyPair.publicKey, keyPair.privateKey, 2048);
    ...
    it('overwrites existing key pair', async () => {
      ...
      await storage.storeKeyPair('test-key', keyPair1.publicKey, keyPair1.privateKey, 2048);
      await storage.storeKeyPair('test-key', keyPair2.publicKey, keyPair2.privateKey, 4096);
    ...
    it('stores multiple key pairs', async () => {
      ...
      await storage.storeKeyPair('key-1', keyPair1.publicKey, keyPair1.privateKey, 2048);
      await storage.storeKeyPair('key-2', keyPair2.publicKey, keyPair2.privateKey, 2048);
  ...
  describe('deleteKeyPair', () => {
    ...
    it('does not throw when deleting non-existent key', async () => {
      await expect(storage.deleteKeyPair('non-existent')).resolves.not.toThrow();
    });
  });
  ...
  describe('listKeyIds', () => {
    ...
    it('returns all stored key IDs', async () => {
      ...
      await storage.storeKeyPair('key-a', keyPair.publicKey, keyPair.privateKey, 2048);
      await storage.storeKeyPair('key-b', keyPair.publicKey, keyPair.privateKey, 2048);
      await storage.storeKeyPair('key-c', keyPair.publicKey, keyPair.privateKey, 2048);
