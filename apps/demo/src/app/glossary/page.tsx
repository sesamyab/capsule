    ...
    <main className="content-page">
      ...
      <nav className="glossary-toc">
        ...
        <ul>
          ...
          <li>
            <a href="#time-buckets">Time Buckets</a>
          </li>
        </ul>
      </nav>
      ...
      <section id="key-hierarchy">
        ...
        <div className="concept-card">
          <h3>Master Secret</h3>
          <p>
            The root secret from which all other keys are derived. Stored
            securely on the subscription server (ideally in a KMS like AWS
            Secrets Manager, HashiCorp Vault, etc.).
          </p>
          ...
          <p className="security-note">
            ⚠️ Never expose the master secret to clients or embed in client
            code.
          </p>
        </div>
        ...
        <div className="concept-card">
          ...
          <p>
            Also called the <strong>Key-Wrapping Key</strong>. Derived from the
            master secret using HKDF with the tier name and bucket ID as
            context. Used to wrap/unwrap DEKs.
          </p>
          <div className="properties">
          ...
          <p>
            When a user unlocks "premium" tier content, they receive the KEK.
            This single key can unwrap the DEK for <em>any</em> premium article
            in that time bucket.
          </p>
...
KEK = HKDF(
  ...
  info: "premium:1737190200"  // tier:bucketId
        ...
        <div className="concept-card">
          ...
          <p>
            The key that actually encrypts the article content. Each article has
            its own unique DEK, generated randomly at encryption time.
          </p>
          <div className="properties">
          ...
          <p>
            The DEK is wrapped with one or more KEKs (tier keys) and stored
            alongside the encrypted content. Clients unwrap the DEK using their
            cached KEK.
          </p>
          <pre className="code-example">
            {`// DEK usage
...
ciphertext = AES-GCM(DEK, plaintext)  // Encrypt content with DEK`}
        ...
        <div className="concept-card">
          ...
          <p>
            Each client generates an RSA-2048 key pair stored in IndexedDB. The
            public key is sent to the server during unlock; the private key
          ...
          <p>
            The server wraps the KEK (or DEK for article keys) with the client's
            public key. Only that specific client can unwrap it with their
            private key.
          </p>
        </div>
      </section>
      ...
      <section id="encryption-algorithms">
        ...
        <div className="concept-card">
          ...
          <p>
            Symmetric block cipher adopted by NIST. Capsule uses AES-256
            (256-bit keys) in two modes:
          </p>
          <ul>
            <li>
              <strong>AES-GCM</strong> (Galois/Counter Mode): For content
              encryption. Provides both confidentiality and authenticity (AEAD).
            </li>
            <li>
              <strong>AES-KW</strong> (Key Wrap): For wrapping DEKs. RFC 3394
              standard for secure key transport.
            </li>
          </ul>
          <div className="properties">
        ...
        <div className="concept-card">
          ...
          <p>
            ...
            Padding. Used for secure key transport from server to client.
          </p>
          <div className="properties">
            <span className="property">Key size: 2048 bits</span>
          ...
          <p>
            The server encrypts the KEK with the client's public key. Only the
            client's private key can decrypt it, ensuring secure key exchange
      ...
      <section id="key-derivation">
        ...
        <div className="concept-card">
          ...
          <p>
            ...
            secret. Capsule uses HKDF-SHA256 to derive KEKs.
          </p>
          <div className="properties">
            <span className="property">Hash: SHA-256</span>
...
Expand:  KEK = HMAC-SHA256(PRK, info || 0x01)`}
          ...
          <p>
            The <code>info</code> parameter includes the tier name and bucket
            ID, ensuring each tier/time combination gets a unique key.
          </p>
        </div>
        ...
        <div className="concept-card">
          ...
          <p>
            Inspired by TOTP (Time-based One-Time Password, RFC 6238), Capsule
            uses time buckets to rotate keys automatically.
          </p>
          <div className="properties">
            <span className="property">
              Bucket period: Configurable (30s demo, longer in production)
            </span>
            <span className="property">Bucket ID: Unix timestamp / period</span>
            <span className="property">
              Window: Current + adjacent buckets valid
            </span>
          </div>
          <p>
            Unlike TOTP which generates short codes, Capsule derives full
            256-bit keys for each bucket. This provides forward secrecy - old
            bucket keys can't decrypt future content.
          </p>
          <pre className="code-example">
            {`// Bucket calculation
...
keyId:     "premium:57906340"`}
      ...
      <section id="key-wrapping">
        ...
        <div className="concept-card">
          ...
          <p>
            RFC 3394 algorithm for securely wrapping cryptographic keys.
            Provides integrity protection - tampering is detected during unwrap.
          </p>
          <div className="properties">
            <span className="property">
              Standard: RFC 3394 / NIST SP 800-38F
            </span>
...
Client (Browser):
  ...
  DEK = AES-KW-Unwrap(KEK, wrappedDek)  // Unwrap DEK locally`}
        ...
        <div className="concept-card">
          ...
          <p>
            The pattern of encrypting data with a DEK, then wrapping the DEK
            with a KEK. This is how Capsule enables "unlock once, access all"
            for tier content.
          </p>
          <ul>
    ...
    { keyId: "article:crypto-basics", wrappedDek: "base64..." }
      ...
      <section id="time-buckets">
        ...
        <div className="concept-card">
          ...
          <p>Time buckets provide several security benefits:</p>
          <ul>
            <li>
              <strong>Forward Secrecy:</strong> Old bucket keys can't decrypt
              new content. If a key is compromised, only that bucket's content
              is at risk.
            </li>
            <li>
            ...
            <li>
              ...
              active subscription to get current bucket keys.
            </li>
          </ul>
        </div>
        ...
        <div className="concept-card">
          ...
          <p>To handle clock differences between server and client, Capsule:</p>
          <ul>
