...
function highlightHtml(code: string): string {
  ...
  code = code.replace(
    /=(&quot;[^&]*?&quot;)/g,
    (match, str) => "=" + placeholder(str, "string")
  );
  code = code.replace(
    /=('[^']*?')/g,
    (match, str) => "=" + placeholder(str, "string")
  ...
  code = code.replace(/(&lt;\/?)([\w-]+)/g, (match, bracket, tagName) => {
    return (
      placeholder(bracket, "punctuation") + placeholder(tagName, "keyword")
    );
  });

  // Closing brackets
  code = code.replace(/(\/?&gt;)/g, (match) =>
    placeholder(match, "punctuation")
  );

  // Attribute names
  code = code.replace(
    /\s([\w-]+)=/g,
    (match, attr) => " " + placeholder(attr, "attr") + "="
  );
